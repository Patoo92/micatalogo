...
// 2. Proteger la página (si no está logueado, fuera)
if (!isset($_SESSION['tienda_id'])) {
    header("Location: login.php");
    exit;
}

// 3. Obtener los datos actuales de la tienda de la base de datos
            ...
            <form action="guardar-configuracion.php" method="POST" enctype="multipart/form-data">
                <div class="mb-3">
                    ...
                    <?php if(!empty($tienda['logo_url'])): ?>
                        <small class="text-muted">Logo actual: <?php echo $tienda['logo_url']; ?></small>
                    <?php endif; ?>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="color" name="color" value="<?php echo $tienda['color_tema'] ?? '#0d6efd'; ?>" class="form-control form-control-color">
                </div>
                
                <div class="mb-3">
                    <label>Número de WhatsApp (ej: +34600123456)</label>
                    <input type="text" name="whatsapp" value="<?php echo htmlspecialchars($tienda['whatsapp_number'] ?? ''); ?>" class="form-control">
                </div>
